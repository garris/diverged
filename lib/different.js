...
function different(reference, test, h, w) {
    ...
    const expandedColumns = ungroupAdjacent(reducedColumnDiff, spread);
...
function groupAdjacent(columns, spread) {
    ...
    function getGroupedValue(columnIndex, depthIndex) {
    ...
    for (let i = 0; i < countOfGroups; i++) {
        ...
        for (let j = 0; j < depth; j++) {
            groupedColumns[i][j] = getGroupedValue(i, j);
        }
    }
    ...
    //         let adjacentValues = [];
    //         for (let adjacentIndex = 0; adjacentIndex <= (spread - 1); adjacentIndex++) {
    //             if (!columns[i + adjacentIndex]) {
    //                 break;
    //             }
    //             adjacentValues.push(columns[i + adjacentIndex][depth]);
    //         }
    //         depthComposite.push(adjacentValues.join('|'));
    //     }
    //     reducedArr.push(depthComposite);
    // }
    // console.log('groupAdjacent>>>', reducedArr, columns)
    // return reducedArr;
    return columns;
}
