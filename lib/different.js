 ...
 */
const spread = 50;
const IS_ADDED_WORD = '0_255_0_225';
const IS_REMOVED_WORD = '255_0_0_225';
const IS_ADDED_AND_REMOVED_WORD = '0_255_255_225';
const IS_SAME_WORD = '255_255_255_225';
function different(reference, test, h, w) {
    console.time("different_total_time");

    // const spread = Math.floor(w*spreadRatio)
...
function groupAdjacent(columns, spread, h, w) {
    ...
    /**
     * [getAdjacentColumnBounds retuns existing adjacent lower and upper column bounds]
     * @param  {[int]} pointer [current index]
     * @param  {[int]} spread  [distance from index]
     * @return {[array]}         [0] lower bound, [1] upper bound
     */
    function getAdjacentColumnBounds(pointer, spread) {
        return [
            // Math.max(0, pointer - spread),
            Math.max(0, pointer),
            Math.min(w - 1, pointer + spread)
        ]
    }
    ...
    let i = 0;
    while (i < w) {
        const adjacentBounds = getAdjacentColumnBounds(i, spread);
