...
function reduceColumnDiffRaw(columnDiffs, h, w) {
    ...
    for (let columnIndex = 0; columnIndex < columnDiffs.length; columnIndex++) {
        ...
        for (let depthIndex = 0; depthIndex < h; depthIndex++) {
            ...
            const pixelAddedClass = columnDiff[depthIndex].added ? IS_ADDED_WORD : IS_SAME_WORD;
            // const pixelRemovedClass = columnDiff[depthIndex].removed ? IS_REMOVED_WORD : IS_SAME_WORD;
            let segment = new Array(columnDiff[depthIndex].count).fill(pixelAddedClass);
